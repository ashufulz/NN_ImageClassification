

## IMAGE CLASSIFICATION - We'll give a 32*32 image to the machine and it will predict what it is about

## LIBRARIES
import cv2 as cv
import matplotlib.pyplot as plt
import numpy as np
from tensorflow.keras import datasets, layers, models

(training_images, training_labels), (testing_images, testing_labels) = datasets.cifar10.load_data()
training_images, testing_images = training_images / 255, testing_images / 255

class_names = ['Plane', 'Car', 'Bird', 'Cat', 'Deer', 'Dog', 'Frog', 'Horse', 'Ship', 'Truck']

'''
for i in range(16):
  plt.subplot(4, 4, i+1)
  plt.xticks([])
  plt.yticks([])
  plt.imshow(training_images[i], cmap=plt.cm.binary)
  plt.xlabel(class_names[training_labels[i][0]])

## USE IF THE DATASET IS LARGE
# training_images = training_images[:20000]
# testing_images = testing_images[:20000]
# training_labels = training_labels[:4000]
# testing_labels = testing_labels[:4000]

## TRAINING THE MODEL
model = models.Sequential()
model.add(layers.Conv2D(32, (3,3), activation='relu', input_shape=(32, 32, 3)))
model.add(layers.MaxPooling2D((2,2)))
model.add(layers.Conv2D(64, (3,3), activation='relu'))
model.add(layers.MaxPooling2D((2,2)))
model.add(layers.Conv2D(64, (3,3), activation='relu'))
model.add(layers.Flatten())
model.add(layers.Dense(64, activation='relu'))
model.add(layers.Dense(64, activation='softmax'))

## COMPILING MODEL
model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

## FITTING MODEL
model.fit(training_images, training_labels, epochs=10, validation_data=(testing_images, testing_labels))

## SAVING THE MODEL
model.save('ImageClassifier.model')

## EVALUATING LOSS AND ACCURACY
loss, accuracy = model.evaluate(testing_images, testing_labels)
print('    Loss:', loss)
print('Accuracy:', accuracy)
'''

## LOADING MODEL
model = models.load_model('ImageClassifier.model')

## READING THE IMAGE GIVEN
img = cv.imread('bird2.jpg')
img = cv.cvtColor(img, cv.COLOR_BGR2RGB)            # COLOR CONVERSION IS NECESSARY ELSE PREDICTION WILL FAIL

plt.imshow(img, cmap=plt.cm.binary)

prediction = model.predict(np.array([img]) / 255)
index = np.argmax(prediction)

## PREDICTING THE PRE GIVEN IMAGE
print('Predicted:', class_names[index])
plt.show()
